CarrierWave.configure do |config|
  ...
  config.aws_bucket = Rails.application.credentials.development[:aws][:s3_bucket_name] # for AWS-side bucket access permissions config, see section below
  ...
  config.aws_credentials = {
    ...
    region:            Rails.application.credentials.development[:aws][:region], # Required
    stub_responses:    Rails.env.test? # Optional, avoid hitting S3 actual during tests
  }
